  ...
  return (
    <div className="p-6 md:p-12 h-[calc(100vh-2rem)] flex flex-col max-w-[1600px] mx-auto">
      <div className="flex justify-between items-center mb-12 shrink-0">
        <div>
          <h1 className="text-4xl font-black text-black dark:text-white tracking-tighter">Kanban de Ordens</h1>
          <p className="text-zinc-500 dark:text-zinc-400 font-bold mt-1 uppercase text-[10px] tracking-widest">Arraste os cards para atualizar o status</p>
        </div>
      </div>

      <div className="flex-1 flex gap-6 overflow-x-auto pb-8 snap-x custom-scrollbar">
        {COLUMNS.map(column => {
          ...
          return (
            <div 
              key={column.id}
              className="flex-1 min-w-[320px] max-w-[400px] flex flex-col bg-zinc-50/50 dark:bg-zinc-900/50 rounded-[2rem] border border-zinc-100 dark:border-zinc-800 overflow-hidden snap-center"
              onDragOver={handleDragOver}
              onDrop={(e) => handleDrop(e, column.id)}
            >
              <div className={`p-6 border-b dark:border-zinc-800 flex items-center justify-between bg-white dark:bg-zinc-900 ${column.textColor}`}>
                <div className="flex items-center gap-3 font-black text-[10px] uppercase tracking-widest">
                  <Icon className="w-5 h-5" />
                  {column.title}
                </div>
                <span className="bg-zinc-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400 px-3 py-1 rounded-full text-[10px] font-black">
              ...
              <div className="flex-1 p-4 overflow-y-auto space-y-4 custom-scrollbar">
                {columnTickets.map(ticket => {
                  ...
                  return (
                    <div
                      ...
                      className="bg-white dark:bg-zinc-900 p-5 rounded-2xl border border-zinc-100 dark:border-zinc-800 card-shadow cursor-grab active:cursor-grabbing hover:border-black dark:hover:border-white transition-all group"
                    >
                      <div className="flex justify-between items-start mb-4">
                        ...
                        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <Link 
                            ...
                            className="p-1.5 text-zinc-300 dark:text-zinc-600 hover:text-black dark:hover:text-white transition-colors"
                            title="Editar"
                          >
                      ...
                      <h3 className="font-black text-black dark:text-white mb-1 line-clamp-2 leading-tight">
                        {client?.name || 'Cliente Desconhecido'}
                      </h3>
                      
                      <div className="space-y-2 mt-4 pt-4 border-t border-zinc-50 dark:border-zinc-800">
                      ...
                      <div className="mt-4 flex justify-end">
                        <Link 
                          ...
                          className="text-[10px] font-black text-black dark:text-white uppercase tracking-widest hover:underline"
                        >
                          Detalhes
                        </Link>
                      </div>
                ...
                {columnTickets.length === 0 && (
                  <div className="h-32 flex items-center justify-center text-zinc-300 dark:text-zinc-700 text-[10px] font-black uppercase tracking-widest border-2 border-dashed border-zinc-100 dark:border-zinc-800 rounded-2xl p-8 text-center">
                    Vazio
                  </div>
                )}
              </div>
