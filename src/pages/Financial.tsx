  ...
  return (
    <div className="p-6 md:p-12 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
        <div>
          <h1 className="text-4xl font-black text-zinc-900 dark:text-white tracking-tighter">Financeiro</h1>
          <p className="text-zinc-500 dark:text-zinc-400 font-bold mt-1 uppercase text-[10px] tracking-widest">Controle de fluxo de caixa</p>
        </div>
        <div className="flex flex-wrap gap-4">
          ...
          >
            <FileSpreadsheet className="w-5 h-5 text-zinc-400 dark:text-zinc-500" /> IMPORTAR EXTRATO
          </label>
          <button 
            onClick={() => {
              setDescription('');
            ...
            className="bg-emerald-600 hover:bg-emerald-700 dark:bg-emerald-700 dark:hover:bg-emerald-600 text-white px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest transition-all active:scale-95 flex items-center gap-2 shadow-lg shadow-emerald-100 dark:shadow-none"
          >
      ...
      {/* Insights Section */}
      {insights && (
        <div className="bg-white dark:bg-zinc-900 p-8 rounded-[2.5rem] border border-zinc-100 dark:border-zinc-800 card-shadow mb-12">
          <h2 className="text-xl font-black text-zinc-900 dark:text-white mb-8 flex items-center gap-3 tracking-tight">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="p-6 bg-zinc-50/50 dark:bg-zinc-800/50 rounded-3xl border border-zinc-50 dark:border-zinc-800">
              <p className="text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest mb-2">Mês Mais Lucrativo</p>
              <p className="text-lg font-black text-zinc-900 dark:text-white">{insights.bestMonth.name}</p>
            ...
            <div className="p-6 bg-zinc-50/50 dark:bg-zinc-800/50 rounded-3xl border border-zinc-50 dark:border-zinc-800">
              <p className="text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest mb-2">Mês com Maior Despesa</p>
              <p className="text-lg font-black text-zinc-900 dark:text-white">{insights.worstMonth.name}</p>
              <p className="text-sm text-red-600 dark:text-red-400 font-black mt-1">
                {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(insights.worstMonth.despesas)}
              </p>
            </div>
            <div className="p-6 bg-zinc-50/50 dark:bg-zinc-800/50 rounded-3xl border border-zinc-50 dark:border-zinc-800">
              <p className="text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest mb-2">Maior Categoria</p>
              <p className="text-lg font-black text-zinc-900 dark:text-white">{insights.topCategory?.name || 'N/A'}</p>
              <p className="text-sm text-amber-600 dark:text-amber-400 font-black mt-1">
                {insights.topCategory ? new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(insights.topCategory.value) : '-'}
              </p>
            </div>
            <div className="p-6 bg-zinc-50/50 dark:bg-zinc-800/50 rounded-3xl border border-zinc-50 dark:border-zinc-800">
              <p className="text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest mb-2">Crescimento</p>
              <div className="flex items-center gap-2">
                <p className="text-lg font-black text-zinc-900 dark:text-white">
                  {Math.abs(insights.growth).toFixed(1)}%
                </p>
                {insights.growth > 0 ? (
                  <span className="flex items-center text-[9px] font-black text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 px-2 py-1 rounded-lg uppercase tracking-wider">
                    <ArrowUpRight className="w-3 h-3 mr-1" /> ALTA
                  </span>
                ) : insights.growth < 0 ? (
                  <span className="flex items-center text-[9px] font-black text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-2 py-1 rounded-lg uppercase tracking-wider">
                    <ArrowDownRight className="w-3 h-3 mr-1" /> QUEDA
                  </span>
                ) : (
                  <span className="text-[9px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-wider">ESTÁVEL</span>
                )}
              </div>
              <p className="text-[10px] font-bold text-zinc-400 dark:text-zinc-500 mt-1">Receitas vs mês anterior</p>
            </div>
          </div>
        </div>
      )}
      ...
      {monthlyData.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
          <div className="bg-white dark:bg-zinc-900 p-8 rounded-[2.5rem] border border-zinc-100 dark:border-zinc-800 card-shadow">
            <h2 className="text-xl font-black text-zinc-900 dark:text-white mb-8 flex items-center gap-3 tracking-tight">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={monthlyData} margin={{ top: 5, right: 5, left: 5, bottom: 5 }}>
                  ...
                  <Tooltip 
                    ...
                    contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)', padding: '12px', backgroundColor: '#18181b', color: '#fff' }}
      ...
      {/* Transactions List */}
      <div className="bg-white dark:bg-zinc-900 rounded-[2.5rem] border border-zinc-100 dark:border-zinc-800 card-shadow overflow-hidden">
        <div className="p-8 border-b border-zinc-50 dark:border-zinc-800">
          <h2 className="text-2xl font-black text-zinc-900 dark:text-white tracking-tight">Histórico</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-zinc-50 dark:divide-zinc-800">
              {transactions.map(t => (
                <tr key={t.id} className="group hover:bg-zinc-50/50 dark:hover:bg-zinc-800/50 transition-colors">
                  <td className="p-6 text-sm text-zinc-400 dark:text-zinc-500 font-bold">
                    {new Date(t.date).toLocaleDateString('pt-BR')}
                  </td>
                  <td className="p-6 text-sm text-zinc-900 dark:text-white font-black">
                    {t.description}
                  </td>
                  ...
                  <td className="p-6 text-center">
                    {t.type === 'expense' && (
                      <button 
                        ...
                        className="p-2 text-zinc-300 dark:text-zinc-600 hover:text-red-600 dark:hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all"
                        title="Excluir Custo"
                      >
      ...
      >
        <form onSubmit={handleAddCost} className="space-y-6">
          ...
          <div>
            <label className="block text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest mb-2">Valor (R$) *</label>
            <input 
              type="number" 
              value={value || ''}
          ...
          <div>
            <label className="block text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest mb-2">Data *</label>
            <input 
              type="date" 
              value={date}
          ...
          <div>
            <label className="block text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest mb-2">Categoria</label>
            <select 
              value={category}
              onChange={(e) => setCategory(e.target.value)}
              className="w-full bg-zinc-50 dark:bg-zinc-800 border-2 border-transparent focus:border-black dark:focus:border-white focus:bg-white dark:focus:bg-zinc-700 rounded-2xl px-6 py-4 outline-none font-bold text-zinc-900 dark:text-white transition-all appearance-none"
            >
      ...
      >
        <form onSubmit={handleAddIncome} className="space-y-6">
          <div>
            <label className="block text-[10px] font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-widest mb-2">Cliente *</label>
            <select 
              value={clientId}
              onChange={(e) => setClientId(e.target.value)}
              className="w-full bg-zinc-50 dark:bg-zinc-800 border-2 border-transparent focus:border-emerald-500 dark:focus:border-emerald-400 focus:bg-white dark:focus:bg-zinc-700 rounded-2xl px-6 py-4 outline-none font-bold text-zinc-900 dark:text-white transition-all appearance-none"
              required
            >
          ...
          <div>
            ...
            <input 
              ...
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              className="w-full bg-zinc-50 dark:bg-zinc-800 border-2 border-transparent focus:border-emerald-500 dark:focus:border-emerald-400 focus:bg-white dark:focus:bg-zinc-700 rounded-2xl px-6 py-4 outline-none font-bold text-zinc-900 dark:text-white transition-all"
          ...
          <div className="pt-4 flex justify-end gap-4">
            ...
            <button 
              type="submit"
              className="bg-emerald-600 text-white px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-emerald-700 transition-all active:scale-95 shadow-lg shadow-emerald-100 dark:shadow-none"
            >
              Salvar Receita
            </button>
          </div>
