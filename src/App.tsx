  ...
  return (
    <Link 
      ...
      className={`flex items-center gap-3 p-4 rounded-2xl transition-all font-bold ${
        isActive 
          ? 'bg-primary text-white shadow-lg shadow-black/20 dark:bg-white dark:text-black' 
          : sub 
            ? 'text-zinc-500 hover:bg-zinc-50 dark:hover:bg-zinc-800 py-3 text-sm'
    ...
    <div className="space-y-1">
      <button 
        ...
        className={`flex items-center justify-between w-full p-4 rounded-2xl transition-all font-bold ${
          hasActiveChild ? 'text-primary dark:text-white' : 'text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800'
        }`}
      >
      ...
      {isOpen && (
        <div className="pl-4 space-y-1 border-l-2 border-zinc-50 dark:border-zinc-800 ml-6">
          {children}
        </div>
      )}
    </div>
...
function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-surface dark:bg-zinc-950 flex flex-col md:flex-row transition-colors duration-200">
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex w-72 bg-white dark:bg-zinc-900 border-r border-zinc-100 dark:border-zinc-800 flex-col shadow-sm z-10 print:hidden">
        <div className="p-8 flex items-center gap-4">
          {companyLogo ? (
            ...
            <div className="w-12 h-12 bg-black dark:bg-white rounded-2xl flex items-center justify-center shadow-lg shadow-black/20">
              <Wrench className="w-6 h-6 text-white dark:text-black" />
            </div>
          )}
          <div className="flex flex-col">
            <span className="font-black text-xl tracking-tighter leading-none dark:text-white">IA COMPANY</span>
            <span className="font-black text-xl tracking-tighter leading-none text-primary dark:text-zinc-400">TEC</span>
          </div>
        </div>
        ...
        <div className="p-6 border-t border-zinc-50 dark:border-zinc-800 space-y-4">
          <button 
            onClick={toggleTheme}
            className="flex items-center gap-3 p-4 rounded-2xl transition-all font-bold text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800 w-full"
          >
          ...
          <button 
            onClick={logout}
            className="flex items-center gap-3 p-4 rounded-2xl transition-all font-bold text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 w-full"
          >
      ...
      {/* Mobile Header */}
      <header className="md:hidden bg-white dark:bg-zinc-900 px-6 py-4 flex justify-between items-center border-b border-zinc-100 dark:border-zinc-800 sticky top-0 z-20">
        <div className="flex items-center gap-3">
          {companyLogo ? (
          ...
          <div className="flex flex-col">
            <span className="font-black text-sm tracking-tighter leading-none dark:text-white">IA COMPANY</span>
            <span className="font-black text-sm tracking-tighter leading-none text-primary dark:text-zinc-400">TEC</span>
          </div>
        </div>
        <button className="p-2 text-zinc-400 hover:text-primary dark:hover:text-white transition-colors">
      ...
      {/* Mobile Bottom Nav */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-zinc-900 border-t border-zinc-100 dark:border-zinc-800 px-4 py-2 flex justify-around items-center z-20 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
